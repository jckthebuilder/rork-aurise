...
struct PhotoBedMissionView: View {
    let onComplete: () -> Void
    ...
    private let accentColor = MissionType.photoBed.accentColor
    ...
    private var completionView: some View {
        VStack(spacing: 16) {
            ...
            if let msg = verificationMessage {
                ...
                Text("First win of the day — bed made!")
                    .font(.subheadline)
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 24) {
                ZStack {
                    ...
                    Image(systemName: step == 0 ? "bed.double.fill" : "camera.fill")
                        .font(.system(size: 48))
                        .foregroundStyle(accentColor)
                }
                ...
                if step == 0 {
                    VStack(spacing: 12) {
                        ...
                        Text("Get up, straighten the sheets, fluff the pillows.\nThis is your first accomplishment today.")
                            .font(.subheadline)
                            .foregroundStyle(.white.opacity(0.6))
                            .multilineTextAlignment(.center)
                            .lineSpacing(4)
                    ...
                    VStack(spacing: 12) {
                        Text("Photograph Your Made Bed")
                            .font(.title2.weight(.bold))
                            .foregroundStyle(.white)

                        Text("Take a photo to prove it's done.\nAI will verify your bed is made.")
                            .font(.subheadline)
                            .foregroundStyle(.white.opacity(0.6))
                            .multilineTextAlignment(.center)
                            .lineSpacing(4)
    ...
    private func simulateSuccess() {
        verificationMessage = "Bed is made! Great start to the day."
    ...
    private func verifyCapture(_ image: UIImage) async {
        ...
        do {
            let result = try await AIService.shared.verifyPhoto(image, target: "", missionType: .madeBed)
            verifying = false
            ...
            print("[PhotoBed] Error: \(error) | Detail: \(detail)")

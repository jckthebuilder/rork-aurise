...
struct PhotoSkyMissionView: View {
    let onComplete: () -> Void
    ...
    private let accentColor = MissionType.photoSky.accentColor
    ...
    private var completionView: some View {
        VStack(spacing: 16) {
            ...
            if let msg = verificationMessage {
                ...
                Text("Sunlight received. You're awake.")
                    .font(.subheadline)
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 24) {
                ZStack {
                    ...
                    Image(systemName: step == 0 ? "sun.horizon.fill" : "camera.fill")
                        .font(.system(size: 48))
                        .foregroundStyle(accentColor)
                }
                ...
                if step == 0 {
                    VStack(spacing: 12) {
                        ...
                        Text("Step outside your door and look up.\nNatural light is the strongest wake signal\nyour body responds to.")
                            .font(.subheadline)
                            .foregroundStyle(.white.opacity(0.6))
                            .multilineTextAlignment(.center)
                            .lineSpacing(4)
                    ...
                    VStack(spacing: 12) {
                        Text("Photograph the Sky")
                            .font(.title2.weight(.bold))
                            .foregroundStyle(.white)

                        Text("Point your camera at the sky and capture\nthis morning's light. AI will verify it.")
                            .font(.subheadline)
                            .foregroundStyle(.white.opacity(0.6))
                            .multilineTextAlignment(.center)
                            .lineSpacing(4)
    ...
    private func simulateSuccess() {
        verificationMessage = "Beautiful sky! Natural light received."
    ...
    private func verifyCapture(_ image: UIImage) async {
        ...
        do {
            let result = try await AIService.shared.verifyPhoto(image, target: "", missionType: .sky)
            verifying = false
            ...
            print("[PhotoSky] Error: \(error) | Detail: \(detail)")

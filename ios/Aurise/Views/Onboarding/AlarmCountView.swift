...
struct AlarmCountView: View {
    @Binding var selection: AlarmCount?
    let onContinue: () -> Void
        ...
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 32) {
                    ...
                    VStack(spacing: 12) {
                        ForEach(Array(AlarmCount.allCases.enumerated()), id: \.element) { index, option in
                            SelectionCard(
                                option.rawValue,
